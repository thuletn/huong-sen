<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<meta http-equiv="refresh" content="2;url=<?php echo base_url();?>thucdon/thucdondoan" >
	<title>Thông báo</title>
	<script rel="stylesheet" href="<?php echo base_url(); ?>vendor/bootstrap.js"></script>
	<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>vendor/bootstrap.css">
	<link rel="stylesheet" type="text/css" href="<?php echo base_url(); ?>vendor/font-awesome.css">
</head>
<body>
	<div class="container">
		<h4 class="text-xs-center">
			<div>
				Bạn đã thao tác <strong>thành công</strong> và sẽ quay lại trang <strong style="color: red;">Quản Lý Đồ Ăn</strong> trong vòng 2 s
			</div>
		</h4>
	</div>
</body>
</html>